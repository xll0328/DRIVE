...
class LaneModule(pl.LightningModule):
    ...
    def __init__(self, model, bs, multitask="angle", dataset="comma", time_horizon=1, ground_truth="desired", intervention=False, dataset_path=None,img_noise =None, dataset_fraction=1.0):
    ...
    def training_step(self, batch, batch_idx):
        ...
        logits, attns,_ = self(image_array, angle, distance, vego)
        ...
        if self.time_horizon > 1:
            ...
            for i in range(self.time_horizon, vego.shape[1], self.time_horizon):
                for j in range(self.time_horizon):
                    ...
                    if self.multitask == "multitask":
                        logits, attns,_ = self(input_ids_img, input_ids_angle, input_ids_distance, input_ids_vego)
                        logits = logits[0][:, -1], logits[1][:, -1]
                    else:
                        logits, attns,_ = self(input_ids_img, input_ids_angle, input_ids_distance, input_ids_vego)[:, -1]
